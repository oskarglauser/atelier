...
/**
 * Z-order used to be the shape's position in the page's Y.Array; it is now the
 * `order` field, so array position and paint order are independent. Backfill
 * from the existing sequence so nothing visibly moves — a static default can't
 * express this, since every shape would collapse to the same value.
 *
 * Text bodies were plain strings replaced wholesale on each edit; they become
 * Y.Text so concurrent edits merge per character instead of last-writer-wins.
 */
function adoptOrderAndRichText(doc: Y.Doc) {
  for (const page of getPages(doc)) {
    const shapesArr = getShapesArray(doc, page.id)
    for (let i = 0; i < shapesArr.length; i++) {
      const map = shapesArr.get(i) as Y.Map<unknown>
      if (typeof map.get('order') !== 'number') map.set('order', i * ORDER_STEP)
      const body = map.get('text')
      if (typeof body === 'string') map.set('text', new Y.Text(body))
    }
  }
}
...
const migrations: Migration[] = [
  ...
  {
    version: 2,
    description: 'Backfill fractional z-order and convert text bodies to Y.Text',
    run: adoptOrderAndRichText,
  },
]
