...
import { getAllShapes, addShape, deleteShapes, reorderShape, groupShapes } from '../document/operations'
...
export async function expandStrokeOnSelection(
  ...
  doc.transact(() => {
    for (const { source, stroke, fill } of jobs) {
      ...
      const sourceIndex = live.findIndex((s) => s.id === source.id)
      ...
      if (!fill) {
        ...
        if (sourceIndex >= 0) reorderShape(doc, pageId, shape.id, sourceIndex)
        ...
        if (gid && sourceIndex >= 0) reorderShape(doc, pageId, gid, sourceIndex)
...
export async function offsetSelectedPaths(
  ...
  doc.transact(() => {
    for (const { source, part } of jobs) {
      ...
      if (sourceIndex >= 0) reorderShape(doc, pageId, shape.id, sourceIndex + 1)
      newIds.push(shape.id)
    }
  }, 'local')
