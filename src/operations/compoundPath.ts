...
import { getAllShapes, addShape, deleteShapes, reorderShape } from '../document/operations'
...
export async function makeCompoundPath(
  ...
  doc.transact(() => {
    ...
    // Take over the topmost input's z position (live index, before deletion)
    const live = getAllShapes(doc, pageId)
    const topIdx = live.findIndex((s) => s.id === topmost.id)
    if (topIdx >= 0) reorderShape(doc, pageId, shape.id, topIdx)
...
export async function releaseCompoundPath(
  ...
  doc.transact(() => {
    const live = getAllShapes(doc, pageId)
    const sourceIndex = live.findIndex((s) => s.id === src.id)
    parts.forEach((part, i) => {
      ...
      // Stack in subpath order where the source sits; rotation/scale are baked
      if (sourceIndex >= 0) reorderShape(doc, pageId, shape.id, sourceIndex + i)
